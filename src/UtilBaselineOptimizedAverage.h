    ...
    void BaselineOptimizedAverage(
      ...
      for ( unsigned int i = 0; i < baselineContainer.size(); i++ )
        ...
        struRigidRegResult result;
          ...
          typedef typename itk::CenteredTransformInitializer<TransformType, itk::Image<float,3> , TImageType> TransformInitializerType;
      ...
      do
        ...
        double         meanIntensity = 0.0;
          ...
          for ( unsigned int i = 0; i < baselineContainer.size(); i++ )
            ...
            newTransSum += ( vcl_abs(currentIterationCurrentImageRegistration.GetTranslationX()) + vcl_abs(currentIterationCurrentImageRegistration.GetTranslationY())
              + vcl_abs(currentIterationCurrentImageRegistration.GetTranslationZ()) ) / 3.0;

            oldRotateSum += ( vcl_abs(allImageRegistrations.at(i).GetFinalAngleInRadiansX()) + vcl_abs(allImageRegistrations.at(i).GetFinalAngleInRadiansY())
              + vcl_abs(allImageRegistrations.at(i).GetFinalAngleInRadiansZ()) ) / 3.0;
            newRotateSum += ( vcl_abs(currentIterationCurrentImageRegistration.GetFinalAngleInRadiansX()) + vcl_abs(currentIterationCurrentImageRegistration.GetFinalAngleInRadiansY())
              + vcl_abs(currentIterationCurrentImageRegistration.GetFinalAngleInRadiansZ()) ) / 3.0;

            std::cout << "Current metric: " << i << " " << currentIterationCurrentImageRegistration.GetMutualInformation() << std::endl;
          ...
          while ( !aIt.IsAtEnd() )
            ...
            InitialAndReturnedAverageBaseline->SetPixel( currLoc , pixelValue); //Replace the baseline with the new estimate here.
            ++aIt;
            }
