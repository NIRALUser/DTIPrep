          ...
          case DWIBaselineAverager::BaselineOptimized:
		    ...
		    Gradient_indx_Baselines.push_back( true );
                  //TODO:  This should use the extract vector element filter.
                    {
                    std::cout << "Extracting element " << i << std::endl;
                    typedef typename itk::VectorIndexSelectionCastImageFilter<TVectorImageType, UnsignedImageType> ExtractBaselineFilterType;
                    typename ExtractBaselineFilterType::Pointer componentExtractor = ExtractBaselineFilterType::New();
                    componentExtractor->SetInput(inputPtr);
                    componentExtractor->SetIndex( i );
                    try
      ...
      for ( unsigned int i = 0; i < this->m_GradientDirectionContainer->size(); i++ )
        {
          ...
          for ( unsigned int j = 0; j < DiffusionDirections.size(); j++ )
            ...
            if ( vcl_abs(this->m_GradientDirectionContainer->ElementAt(i)[0] - DiffusionDirections[j].gradientDir[0]) < NearZeroSmallNumber
              && vcl_abs(this->m_GradientDirectionContainer->ElementAt(i)[1] - DiffusionDirections[j].gradientDir[1]) < NearZeroSmallNumber
              && vcl_abs(this->m_GradientDirectionContainer->ElementAt(i)[2] - DiffusionDirections[j].gradientDir[2]) < NearZeroSmallNumber )
              {
