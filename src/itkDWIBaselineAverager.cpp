	...
	*/
		...
		::average()
	{
		...
		else
		{
			...
			if ( this->baselineNumber == 1 )   // single baseline: copy baseline into
				// averagedBaseline for getBaseline()
			{
				DirectAverage();
				this->computeIDWI();
			}
		...
		::GradientOptamizedAverage()
	{
		...
		do
		{
				...
				i++ )
			{
				...
				|| 0.0 !=  this->m_GradientDirectionContainer->ElementAt(i)[2]    )
				{
					...
					rigidRegistration( this->idwi,
						componentExtractor->GetOutput(), 25, 0.1, 1, resultLocal, 1 /* DW
																					gradient
																					*/);
			...
			while ( !idwiIt.IsAtEnd() )
			{
				...
				idwiIt.Set( pow( tempIDWI->GetPixel(pixelIndex), 1.0
					/ this->getGradientNumber() ) );
				meanSquarediff
					+= ( pixelValue - idwiIt.Get() ) * ( pixelValue - idwiIt.Get() );
		...
		for ( unsigned int i = 0; i < this->m_GradientDirectionContainer->Size(); i++ )
		{
			...
			&& 0.0 ==  this->m_GradientDirectionContainer->ElementAt(i)[2]    )
			{
				...
				rigidRegistration( this->idwi,
					componentExtractor->GetOutput(), 25, 0.1, 1, resultLocal, 0 /* baseline
																				*/);
				//         std::cout<<" done "<<std::endl;
			}
		}
		...
		::BaselineOptimizedAverage()
	{
		...
		do
		{
			...
			for ( unsigned int i = 0; i < baselineContainer.size(); i++ )
			{
				...
				// baseline ... ";
				rigidRegistration( this->averagedBaseline,
					baselineContainer[i],
					25,
					0.1,
					1,
				...
				oldTransSum
					+= ( fabs(Results.at(i).TranslationX)
					+ fabs(Results.at(i).TranslationY)
					+ fabs(Results.at(i).TranslationZ) ) / 3.0;
				newTransSum
					+= ( fabs(resultLocal.TranslationX) + fabs(resultLocal.TranslationY)
					+ fabs(resultLocal.TranslationZ) ) / 3.0;

				oldRotateSum
					+= ( fabs(Results.at(i).AngleX) + fabs(Results.at(i).AngleY)
					+ fabs(Results.at(i).AngleZ) ) / 3.0;
				newRotateSum
					+= ( fabs(resultLocal.AngleX) + fabs(resultLocal.AngleY)
		...
		while ( !aIt.IsAtEnd() )
		{
			...
			for ( unsigned int i = 0; i < inputPtr->GetVectorLength(); i++ )
			{
					...
					&& this->m_GradientDirectionContainer->ElementAt(i)[2] < 1e-7     )
				{
					...
					/ ( static_cast<double>( getBaselineNumber() ) );
				}
			}
	...
	*/
		...
		::writeReport()
	{
		...
		case DWIBaselineAverager::REPORT_TYPE_EASY_PARSE:
			...
			outfile << "Baseline_Avg\tbValueNumber "    << bValueNumber    << std::endl;
			outfile << "Baseline_Avg\tbaselineNumber "  << baselineNumber  << std::endl;
			outfile << "Baseline_Avg\tgradientDirNumber " << gradientDirNumber  << std::endl;
			outfile << "Baseline_Avg\trepetitionNumber " << repetitionNumber  << std::endl;
	...
	*/
		...
		::collectDiffusionStatistics()
	{
		...
		for ( unsigned int i = 1; i < repetNum.size(); i++ )
		{
			...
			{
				std::cout
					<< "Warring: Not all the gradient directions have same repetition. "
					<< std::endl;
				repetitionNumber = -1;
			}
		}
	...
	*/
		...
		)
	{
		...
		typedef itk::VersorRigid3DTransformOptimizer
			OptimizerType;
		typedef itk::MattesMutualInformationImageToImageMetric<UnsignedImageType,
			UnsignedImageType> MetricType;
		typedef itk::LinearInterpolateImageFunction<UnsignedImageType,
			double>            InterpolatorType;
		typedef itk::ImageRegistrationMethod<UnsignedImageType,
			UnsignedImageType> RegistrationType;

		typedef UnsignedImageType::SpacingType
			SpacingType;
		typedef UnsignedImageType::PointType
			OriginType;
		typedef UnsignedImageType::RegionType
			RegionType;
		typedef UnsignedImageType::SizeType
			SizeType;
		...
		typedef itk::CenteredTransformInitializer<TransformType, UnsignedImageType,
			UnsignedImageType> TransformInitializerType;
		...
		// std::cout<<"UseNumberOfSpatialSamples:
		// "<<(int)(fixedImage->GetPixelContainer()->Size() *
		// percentOfSamples)<<std::endl;
		int SampleSize
		...
		typedef itk::ResampleImageFilter<UnsignedImageType,
			UnsignedImageType> ResampleFilterType;
		...
		typedef ImageRegionIteratorWithIndex<UnsignedImageType>
			transformedImageIterator;
		transformedImageIterator transIt( transformedImage,
			transformedImage->GetLargestPossibleRegion() );
		transIt.GoToBegin();
		typename UnsignedImageType::IndexType pixelIndex;
		while ( !transIt.IsAtEnd() )
		{
			...
			{
				tempBaseline->SetPixel( pixelIndex, tempBaseline->GetPixel(
					pixelIndex ) + (int)(transIt.Get() + 0.5) );
			}
			else   // DW gradient
			{
				tempIDWI->SetPixel( pixelIndex, tempIDWI->GetPixel(
					pixelIndex ) * transIt.Get() );
			}
	...
	*/
		...
		::computeIDWI()
	{
		...
		while ( !idwiIt.IsAtEnd() )
		{
			...
			idwiIt.Set( static_cast<unsigned int>( pow( pixelValue, 1.0
				/ this->getGradientNumber() ) ) );
			++idwiIt;
		}
	}
