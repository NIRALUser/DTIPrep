...
namespace itk
{
	...
	*/
		...
		::DWIEddyCurrentHeadMotionCorrector()
	{
		...
		m_ReportFileName = "";
		m_ReportFileMode = DWIEddyCurrentHeadMotionCorrector::Report_New;
	...
	*/
		...
		::GenerateOutputInformation()
	{
		...
		{
			std::cout << "ERROR: Gradient # and Diffusion Direction # mismatch!"<< std::endl;
			return;
		}
...
// 			itk::EncapsulateMetaData<std::string>( outputMetaDictionary, "NRRD_thickness[2]", "NaN");
	...
	*/
		...
		::writeReport()
	{
		...
		if( GetReportFileMode()== DWIEddyCurrentHeadMotionCorrector::Report_Append )
		...
		outfile<<"  m_MaxNumberOfIterations: "<< m_MaxNumberOfIterations<<std::endl;
			...
			if(DiffusionDirHistOutput.size()>0)
			{
				...
				for(int j=0;j<DiffusionDirHistOutput.size();j++)
				{
					if( this->re.GetGradients().at(i)[0] == DiffusionDirHistOutput[j].gradientDir[0] && 
						...
						this->re.GetGradients().at(i)[2] == DiffusionDirHistOutput[j].gradientDir[2] )
					{
