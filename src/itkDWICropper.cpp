...
namespace itk
{
	...
	*/
		...
		::GenerateOutputInformation()
	{
		...
		typename InputImageType::RegionType imageOriginalRegion = inputPtr->GetLargestPossibleRegion();
		typename InputImageType::SizeType imageOriginalSize = imageOriginalRegion.GetSize();
...
// 			itk::EncapsulateMetaData<std::string>( outputMetaDictionary, "NRRD_thickness[2]", "NaN");
	...
	*/
		...
		::writeReport()
	{
		...
		if( GetReportFileMode()== DWICropper::Report_Append )
		...
		outfile <<"input  size: [ "<< imageOriginalSize[0] <<", "<< imageOriginalSize[1] <<", "<< imageOriginalSize[2] <<" ] "<<std::endl;
		outfile <<"output size: [ "<< region[3] <<", "<< region[4] <<", "<< region[5] <<" ] "<<std::endl;
		outfile <<"start index: [ "<< region[0] <<", "<< region[1] <<", "<< region[2] <<" ] "<<std::endl;
	...
	*/
		...
		::collectDiffusionStatistics()
	{
		...
		{ 
			...
			{
				std::cout<<"Warnning:Not all the gradient directions have same repetition. "<<std::endl;
				repetitionNumber = -1;			
			}
		}
	...
	*/
		...
		::ThreadedGenerateData(const OutputImageRegionType& outputRegionForThread,int threadId )
	{
		...
		typename TImageType::RegionType imageOriginalRegion = inputPtr->GetLargestPossibleRegion();
