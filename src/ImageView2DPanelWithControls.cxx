...
ImageView2DPanelWithControls::ImageView2DPanelWithControls(QString title, QMainWindow *parent):QDockWidget(title, parent)
{
  setupUi(this);
...
void ImageView2DPanelWithControls::Setup( vtkImageData *image, double ww,double wl, int SliceIndex,int orient, int numGradients, int id)
{
	if(id>=0 && id<=2) WindowID=id;
	else WindowID=0;
	...
	//std::cout<<ImageDimension[0]<<"    "<< ImageDimension[1]<<"     "<<ImageDimension[2]<<std::endl;
	//std::cout<<ImageSpacing[0]<<"    "<< ImageSpacing[1]<<"     "<<ImageSpacing[2]<<std::endl;
	this->ImageViewer2->SetRenderWindow(qvtkWidget->GetRenderWindow());
	this->ImageViewer2->SetupInteractor(qvtkWidget->GetRenderWindow()->GetInteractor() );
	...
	switch(Orientation)
	{
		...
		case ORIENTATION_SAGITTAL:
			...
			this->ImageViewer2->GetRenderer()->GetActiveCamera()->SetPosition(-1,0,0); // -1 if medical ?
		...
		case ORIENTATION_CORONAL:
			...
			if(index<0) index = ImageDimension[1]/2;
			this->ImageViewer2->GetRenderer()->GetActiveCamera()->SetPosition(0,-1,0); // 1 if medical ?
			this->ImageViewer2->GetRenderer()->GetActiveCamera()->SetViewUp(0,0,1);
	...
	if(ImageDimension[0]!=0 && ImageDimension[1]!=0 && ImageDimension[2]!=0)
	{
		...
		if(ImageDimension[0]*ImageSpacing[0]>=ImageDimension[2]*ImageSpacing[2]) 
			scaleTemp[1]= ImageDimension[0]*ImageSpacing[0]/2.0;
		else scaleTemp[1]= ImageDimension[2]*ImageSpacing[2]/2.0;

		if(ImageDimension[1]*ImageSpacing[1]>=ImageDimension[2]*ImageSpacing[2])
			scaleTemp[2]= ImageDimension[1]*ImageSpacing[1]/2.0;
		else scaleTemp[2]= ImageDimension[2]*ImageSpacing[2]/2.0;
...
void ImageView2DPanelWithControls::on_horizontalSlider_SliceIndex_valueChanged(int index)
{
	QString str;
	//this->lineEdit_SliceIndex->setText(str.sprintf("%d/%d", index, horizontalSlider_SliceIndex->maximum()));
	if(ImageDimension[2]==0) return;
...
void ImageView2DPanelWithControls::on_comboBox_Orientation_currentIndexChanged(int indexOrient)
{
	if( Orientation == indexOrient) return;
	if(!bSetup) return;
...
void ImageView2DPanelWithControls::on_comboBox_Contents_currentIndexChanged(int index)
{
...
{	
	if(index>0) 	ImageViewer2->GetImageActor()->InterpolateOn();
	else 		ImageViewer2->GetImageActor()->InterpolateOff();
...
void ImageView2DPanelWithControls::on_horizontalSlider_Gradient_valueChanged(int index)
{
	if(NumberOfGradients==0) return;
	QString str;
	//this->lineEdit_SliceIndex->setText(str.sprintf("%d/%d", index, horizontalSlider_SliceIndex->maximum()));
...
void ImageView2DPanelWithControls::on_toolButton_windowlevel_Syn_toggled(bool syn)
{
	emit SynWindowLevel(syn);
}

void ImageView2DPanelWithControls::on_toolButton_interpolation_Syn_toggled(bool syn)
{
	emit SynInterpolation(syn);
}

void ImageView2DPanelWithControls::on_toolButton_orientation_Syn_toggled(bool syn)
{
