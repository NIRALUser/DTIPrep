  ...
  if ( getBValueNumber() >= 3
    ...
    for ( unsigned int i = 0; i < this->ResultsContainer.size(); i++ )
      ...
      if ( fabs(this->ResultsContainer[i].AngleX) > m_RotationThreshold
           ...
           || this->ResultsContainer[i].Correlation <
           m_CorrelationThresholdGradient
           || normalizedMetric[i] < quardraticFittedMeans
           - quardraticFittedDeviations * m_CorrelationStdevTimesGradient )
        {
      ...
      if ( this->m_GradientDirectionContainer->at(i)[0] == 0.0
        ...
        if ( fabs(this->ResultsContainer[i].AngleX) > m_RotationThreshold
             ...
             || fabs(this->ResultsContainer[i].TranslationZ - 0.5
               * this->GetInput()->GetSpacing()[2]) > m_TranslationThreshold
             || this->ResultsContainer[i].Correlation <
             m_CorrelationThresholdBaseline
             || this->ResultsContainer[i].Correlation <
             interlaceBaselineMeans - interlaceBaselineDeviations
        ...
        if ( fabs(this->ResultsContainer[i].AngleX) > m_RotationThreshold
             ...
             m_CorrelationThresholdGradient
             || this->ResultsContainer[i].Correlation <
             interlaceGradientMeans - interlaceGradientDeviations
  ...
  ::writeReport()
{
  ...
  //     outfile<< "interlaceBaselineDeviations: " <<
  // interlaceBaselineDeviations<<std::endl;
  //     outfile<< "interlaceBaselineMeans:      " <<
  // interlaceBaselineMeans<<std::endl;
  //
  //     outfile<< "interlaceGradientDeviations: " <<
  // interlaceGradientDeviations<<std::endl;
  //     outfile<< "interlaceGradientMeans:      " <<
  // interlaceGradientMeans<<std::endl;
