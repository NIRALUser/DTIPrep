...
double DiffusionTensorEstimation::EstimateTensor_Whitematter_GrayMatter(std::string buffer1, std::string buffer2, std::string buffer3 /*, std::string buffer2 */, std::string buffer4)
{
			...
			if ( buffer4.length()==0 )		// this condition was used for computing entropy value from given tensor, if buffer4 is empty means that required tensor computed from dwi image (buffer3)
			{
				...
				str8 << "--dwi_image" << QString::fromStdString(buffer3.c_str()) << "-m" << "wls" << "-M" << QString::fromStdString(buffer1.c_str()) << "--tensor_output" << "tensor.nrrd" << "-t" << "0";
...
/*******************************************************************************************************************************/
void DiffusionTensorEstimation::SaveEntropyResult_beta( std::string Reportfilename, std::string Labelfilename, std::string DWIfilename)	
{
