...
double Counter_NearestNeighborVertex::Calculation_SubAreaBin( std::vector <double> Sides, IndexList indices, short Source)
{
	...
	double a_adj = sqrt( (double) (v3[0]-v2[0])*(v3[0]-v2[0])+(v3[1]-v2[1])*(v3[1]-v2[1])+(v3[2]-v2[2])*(v3[2]-v2[2]) );
	double b_adj = sqrt( (double) (v1[0]-v3[0])*(v1[0]-v3[0])+(v1[1]-v3[1])*(v1[1]-v3[1])+(v1[2]-v3[2])*(v1[2]-v3[2]) );
		...
		{	
			for ( short k = 0 ; k < 3 ; k++ )
			{
				if ( i == icosahedron->GetTriangleVertices(j)[k] )
				{
					...
					outfile << "index " << i << " Triangle " << j << " " << Calculation_SubAreaBin ( SideTriangle, icosahedron->GetTriangleVertices(j), i )<< std::endl;
