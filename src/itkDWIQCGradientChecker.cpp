...
namespace itk
{
	...
	*/
		...
		::DWIQCGradientChecker()
	{
		...
		b0 = -1.0 ;
		m_ReportFileName = "";
		m_ReportFileMode = DWIQCGradientChecker::Report_New;
		m_ReferenceIndex = 0;
		bValues.clear();
...
// 			std::cout<<std::endl<<"Register Gradient "<<j<<" to Baseline or first Image ..."<<std::endl;

			rigidRegistration(componentExtractor->GetOutput(),componentExtractor1->GetOutput(), 25, 0.1, 1, result );
			this->ResultsContainer.push_back(result);

			std::cout<<".";

// 			std::cout<<"Angles: "<<this->ResultsContainer[j-1].AngleX<<" "<<this->ResultsContainer[j-1].AngleY<<" "<<this->ResultsContainer[j-1].AngleZ<<std::endl;
// 			std::cout<<"Trans : "<<this->ResultsContainer[j-1].TranslationX<<" "<<this->ResultsContainer[j-1].TranslationY<<" "<<this->ResultsContainer[j-1].TranslationZ<<std::endl;
// 			std::cout<<"MI    : "<<this->ResultsContainer[j-1].MutualInformation<<std::endl;
		}
...
// 		std::cout<<"m_TranslationThreshold: "<<m_TranslationThreshold<<std::endl;
		...
		for(int i=0;i< this->ResultsContainer.size();i++)
		{
			if( fabs(this->ResultsContainer[i].AngleX) > m_RotationThreshold		  || fabs(this->ResultsContainer[i].AngleY) > m_RotationThreshold			||
				...
				fabs(this->ResultsContainer[i].TranslationY) > m_TranslationThreshold || fabs(this->ResultsContainer[i].TranslationZ) > m_TranslationThreshold     )
			{
				this->qcResults[i] = 0;
			}
		}
	...
	*/
		...
		::writeReport()
	{
		...
		if( GetReportFileMode()== DWIQCGradientChecker::Report_Append )
		...
		outfile <<"Gradient-wise motion check with MI-based 3D rigid registration: "<<std::endl;
